...
class SatelliteCNNEncoder:
    def __init__(
        ...
        self.encoder = None
        self.valid_ids = self._get_ids_with_images()

    def _get_ids_with_images(self):
        df = pd.read_csv(self.id_source_csv)
        df[self.id_col] = df[self.id_col].astype(str).str.strip()
        return [
            i for i in df[self.id_col].values
            if os.path.exists(os.path.join(self.image_dir, f"{i}.jpg"))
        ]

    def _data_generator(self):
        price_df = pd.read_csv(self.price_csv_path)
        price_df[self.id_col] = price_df[self.id_col].astype(str).str.strip()
        id_to_y = dict(zip(price_df[self.id_col], price_df[self.target_col]))
    ...
    def generate_embeddings(self, model):
        ...
        for i in ids:
            ...
            if not os.path.exists(path):
                ...
                emb = model.predict(img[None, ...], verbose=0)[0]
